...
`FieldContext` = `{goal, field{label, role, value}, page{title, text ≤ 6000}, recent_actions[last 6]{action, text}}` (+ `voice` for free-text fields: `textarea`/contenteditable). It derives `PartialEq`; that equality **is** the cache rule. Helper output must parse as an object whose only key is `text`, a non-blank string ≤ 2,000 chars; `null` → `Unknown`; anything else → error, nothing typed. Default impl calls the task's snapshotted `AgentRuntime::complete_json` using that runtime's lowest-cost/fastest strict-JSON-capable model (prefer eligible `luna`, reasoning off), max 1,024 output tokens. This is direct Responses on the API runtime or app-server `outputSchema` on the ChatGPT runtime *(verify exact no-reasoning mapping in both catalogs)*. The helper never sees, and is never asked for, a secret.
