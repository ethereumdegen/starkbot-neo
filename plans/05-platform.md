...
starkbot-neo/
  ...
  crates/
    ...
    neo-media/  neo-canvas/  neo-canvas-agent/  neo-extension-host/  neo-agent/  neo-cli/  neo-tui/
...
| `neo-keys` | Keychain read/write/delete, the `Secret` newtype, `KeyAccount`, `KeyStatus`, the `KeyValidator` trait, env fallback. The only place a **Starkbot-owned** secret exists — the bare keys `openai`, `anthropic`, `typesafe` (K6) and the two subscription OAuth credential blobs `anthropic-oauth`, `openai-codex` (K7). The two vendor-CLI fallbacks still own their own credentials in their own homes. Leaf crate: no `neo-*` dependencies, no HTTP, no vendor URLs. |
...
| `neo-media` | *(removed 2026-09-19, A12′ — media via apps)* Adapter over the `degen-media-maker` lib: media tools, `MediaBackend` wiring (`fal`, `quiver`), spend estimates, studio index, fal/Quiver `KeyValidator`s. |
...
| `neo-agent` | Queue worker, router, `AgentRuntime` in its six implementations — `MetalcraftRuntime<OpenAiInference>`, `MetalcraftRuntime<AnthropicInference>`, `MetalcraftRuntime<AnthropicOauthInference>`, `MetalcraftRuntime<CodexOauthInference>` (A25) and the two fallbacks `CodexRuntime`, `ClaudeCodeRuntime` (A22) — the `oauth` module (PKCE flow, loopback callback, token exchange/refresh, `OauthStore` over `neo-keys`), the two fallback helper supervisors, Sol tools behind one `ToolDispatcher`, `Gated<T>`, caps, trace, spend/allowance meter, and **`Runtime`** — the one facade (`start`, commands in, `AppEvent` stream out) that the CLI, the TUI and the desktop app all drive. |
| `neo-cli` | The `neo` binary: every subsystem headless (`doctor`, `keys`, `models`, `voice`, `nav`, `judge`, `run`, `pack`, `media`, `canvas`, `scenario`), `neo tui` which hands off to `neo-tui` (P12), plus hidden `neo dev …` maintenance commands (export bindings, capture fixtures, seed DBs, fetch pinned vendor helpers). |
| `neo-tui` | The ratatui front end behind `neo tui` (P12): panes for Conversation, Queue, Mind/trace and confirm cards, drawn from the `AppEvent` stream and sending typed commands back. Thin exactly like the webview — no secret, no decision, no product logic. No `neo-*` dependency beyond `neo-agent` (the `Runtime` facade) and `neo-core` (the shared types); never `neo-store`, never Tauri. Doc 14 owns its layout, key map and event loop. |
...
neo-agent ─▶ all of the above (+ metalcraft, rig; supervises the pinned Codex and Claude Code helpers over stdio)
neo-cli ─▶ neo-tui ─▶ neo-agent                 (the "neo tui" subcommand is the TUI entry point)
src-tauri ─▶ neo-agent
```
...
2. **Starkbot-owned secrets only in `neo-keys`.** `Secret`'s inner string is private; `Secret::expose()` is on `clippy.toml`'s `disallowed-methods` and is `#[allow]`ed at exactly five call sites: provider impls building an auth header, `jev-nav` credential adapter in `neo-judge`, the pack HTTP runner's `$VAR` expansion, the rig client constructors in `neo-agent` (OpenAI Responses and Anthropic Messages), and the OAuth credential boundary in `neo-agent::oauth` (token exchange/refresh body and the `Authorization` header it mints). Headers carrying a key or an access token are `HeaderValue::set_sensitive(true)`. The two **fallback** helpers' tokens never cross their local stdio protocol (K6); the two **primary** subscription paths hold their OAuth credential in `neo-keys` and nowhere else (K7).
3. **No vendor URL outside a provider impl** (or, for packs, outside pack data). CI greps for `api.openai.com`, `api.anthropic.com`, `typesafe.ai`, `fal.run`, `quiver.ai` outside the allow-listed modules; the same grep denies any vendor hostname or secret-shaped literal anywhere in `neo-tui` or `ui/` — a front end never names a host and never holds a key (P12).
4. **No Python, no shell agent surface**: no `.py` file, no `python` invocation anywhere in the repo or CI (a CI step fails on either).
5. `neo-core` does no I/O; `neo-store` is the only SQL; `src-tauri`, `ui/` and `neo-tui` hold no secrets and no logic. Provider impls and HTTP clients take `{ base_url, credential }` from their provider object (08); tests point them at `wiremock`.
...
| Terminal UI | `ratatui` **0.30** (default `crossterm` backend; `ratatui-core` / `ratatui-crossterm` arrive through it and are never pinned separately) · `crossterm` **0.29** (feature `event-stream` for the tokio event source); buffer snapshots use the shared `insta` |
...
### 4.3 Schema (final shape through `migrations/0003_runtime_scoped_usage.sql`)
...
CREATE TABLE provider_accounts (                                           -- redacted status only; credentials remain in provider-owned Keychain storage
  provider TEXT PRIMARY KEY, status TEXT NOT NULL CHECK (status IN ('signed_out','connected','rate_limited','unavailable')),
  email TEXT, plan_type TEXT, workspace TEXT, allowance TEXT CHECK (json_valid(allowance)), updated_at INTEGER NOT NULL) STRICT;
...
CREATE TABLE pack_keys (                                                    -- status only; the value is in the Keychain. pack_id '@core' = openai, anthropic, typesafe, anthropic-oauth, openai-codex (K7)
...
- **Key accounts are an open set, and there are five core ones**: the bare keys `openai`, `anthropic`, `typesafe` (K6) and the two **subscription OAuth credential blobs** `anthropic-oauth`, `openai-codex` (K7) · any `requires_env` name a user-installed pack declares · later `starkrouter`. Pack accounts are the env name itself, so two packs that declare the same name share one item—but only after the consent screen names the second recipient. Core accounts are not env-style names, so no pack can name them. `$VAR` expands only for names the pack declares, and only into requests to that integration's `allowed_hosts`. The two OAuth accounts hold **JSON, not a bare key** — `{access_token, refresh_token, expires_at_ms, account_id?, email?, plan?}`, serialised as one item per provider, read through `Secret`, rewritten on every refresh, and never expanded as a `$VAR`. The **vendor-CLI fallbacks** are still not `neo-keys` accounts: the Codex helper keeps its tokens in its own `CODEX_HOME` keyring namespace and Claude Code in the Keychain entry keyed to its `CLAUDE_CONFIG_DIR` (08, K6). Media-generation credentials likewise remain in the media apps (K1′, K4).
- **`Secret`**: private `String`, `Zeroize` on drop, `Debug`/`Display` print `Secret(•••• last4)`, no `Serialize`, no `Clone` outside the crate (`Arc<Secret>` is shared). `expose()` is clippy-disallowed except at the five sites in §1 rule 2. Secrets are read once and cached in-process so the Keychain is not hit per request; an OAuth blob's cache entry is replaced, not appended to, when a refresh lands.
...
| `anthropic` | `GET {base}/v1/models` with `x-api-key` and `anthropic-version: 2023-06-01` | 200 → `present`; if the model the configured Anthropic inference use case needs is absent → `limited`; 401 → `invalid` |
| `anthropic-oauth` (K7) | refresh the stored credential against the provider's injected `token_url`, then one identity read (08) | a refreshed credential → `present`; `invalid_grant` / 401 → `invalid`, the row goes `signed_out` and the user re-runs **Connect Claude** |
| `openai-codex` (K7) | refresh the stored credential against the provider's injected `token_url` (08) | a refreshed credential → `present`; `invalid_grant` / 401 → `invalid`, re-run **Connect ChatGPT** |
...
- **Subscription account flows (K7, superseding the custody half of K6)** — inference only, and now a **Starkbot-owned** credential. *Primary, both vendors*: `OauthFlow::start(provider)` generates the PKCE verifier (S256) and a hex `state`, opens the vendor's authorize URL in the default browser, and serves the loopback callback — `localhost:54545/callback` for `anthropic-oauth`, `localhost:1455/auth/callback` for `openai-codex` — until the code arrives; a pasted redirect URL is the fallback when the browser cannot reach this machine. `OauthClient::exchange` turns the code into the credential blob, which goes straight into the Keychain through `neo-keys`; `OauthStore::access_token` refreshes in-process whenever the access token is within five minutes of expiry, and the refreshed blob replaces the item. An `id_token`, where the vendor returns one, is parsed for the account id (`ChatGPT-Account-Id`) and the email only — it is never trusted for an authorization decision. *Fallback, both vendors*: `CodexSupervisor` (dedicated `CODEX_HOME`) and `ClaudeCodeSupervisor` (dedicated `CLAUDE_CONFIG_DIR` *(verify)*) still let the vendor's own binary own login, refresh and storage, and no token crosses that stdio boundary. For **every** path, Neo persists redacted `{status, email?, plan_type?, workspace?, allowance?, updated_at}` in `provider_accounts` only: no access or refresh token, and no `id_token`, ever reaches SQLite, settings, an `AppEvent`, a trace, a log, a diagnostic bundle, a snapshot test, the TUI or the webview. Disconnect deletes the Keychain account (or calls the helper's own logout) and clears the row, and never touches the user's unrelated Codex or Claude Code sessions.
- **Env fallback**: `KeySource::KeychainThenEnv` reads `OPENAI_API_KEY`, `ANTHROPIC_API_KEY` and `TYPESAFE_API_KEY`; user-installed packs may read only env names declared in their own `requires_env`. `neo` and `neo tui` enable fallback; the app uses it only under `cfg!(debug_assertions)`. A release app never reads a key from the environment. **No subscription credential is ever read from the environment or imported from another program's credential store** — not from OMP, not from `~/.codex`, not from `~/.claude` — and one connection never borrows another's credential (K6, K7).
...
- **Tiers, not vendor words.** Starkbot reasons in three tiers — **Sol** (the orchestrator and creative), **Terra** (middle) and **Luna** (cheapest/fastest, the text helper's home). OpenAI spells them `sol`/`terra`/`luna`; Anthropic spells them `opus`/`sonnet`/`haiku`, and they map onto the same three tiers, so `sol-latest` and the text-helper default mean the same thing on either API runtime (K6). The mapping and the ordering live in `neo-core::registry`.
...
| Runtime | Use case | Pattern |
|---|---|---|
| `openai` | inference / text helper | `^gpt-(\d+(?:\.\d+)*)-(sol\|terra\|luna)(-\d{4}-\d{2}-\d{2})?$` → `{version, tier, dated}` |
| `openai` | STT · streaming STT | `^gpt-transcribe` · `^gpt-live-transcribe` |
| `openai` | TTS | `-tts(-\|$)` |
| `anthropic` | inference / text helper | `^claude-…` with a tier word (`opus`/`sonnet`/`haiku`) anywhere and every numeric segment read as the version; an 8-digit segment is a snapshot date, and `latest`/`preview` counts as dated *(verify the tier names and id shapes against the live Anthropic catalogue)* |
| `anthropic` | speech | **none** — speech is OpenAI-API-key only (K6) |
| any | never offered | hide-list `whisper-1`, `gpt-4o-transcribe*`, `gpt-4o-mini-transcribe*`, `tts-1*` (K3); `gpt-image*`, `dall-e*` (K4); everything unclassified |

- **`sol-latest` is runtime-scoped**. For a raw API catalog it means the undated Sol-tier id with the highest version, compared as numeric tuples (`5.10 > 5.9`) — `gpt-*-sol` on `openai`, `claude-opus-*` on `anthropic`; previews and dated snapshots are excluded. For Codex it means the highest eligible Sol model the connected account's catalog marks available. Settings store the symbolic value plus runtime; the UI shows the resolved id beside it. A changed resolution applies from the next task and posts one `notice` message ("Sol is now …") with new API price or current plan-allowance state.
...
- **Identity**: `bundle.macOS.signingIdentity = "Developer ID Application: …"`, hardened runtime, universal binary (`--target universal-apple-darwin`), **not sandboxed**, `app.macOSPrivateApi = true`. Minimum macOS 14 *(D)*. `neo` ships inside the bundle as a signed nested executable, as does every pinned official vendor helper whose licence allows redistribution (§9.1); release CI verifies each architecture, upstream commit, SHA-256, byte count and licence digest before signing.
...
```
  ...
  Deliberately absent: `app-sandbox`, `cs.allow-unsigned-executable-memory`, `cs.disable-library-validation`, `cs.allow-dyld-environment-variables`, `automation.apple-events`, `device.camera`. `entitlements.debug.plist` adds only `com.apple.security.get-task-allow` (notarization rejects it, so it never reaches a release). Each nested vendor helper is signed with hardened runtime and no broader entitlement; the empty scratch working directory, disabled shell/file/command tools and protocol deny rules A22 requires are application controls, not substitutes for signing.
...
- **Updater**: `tauri signer generate` keypair; the public key in `tauri.conf.json`; the private key + password in CI secrets **and two offline backups — losing it strands every install**. Static `latest.json` + `.app.tar.gz` + `.sig` on the release host. Check on launch and every 24 h; download in the background; **install only when no task is running, the queue is empty or paused, no confirm is pending, and every vendor helper subprocess has exited**, then relaunch restoring Listen. Same Developer ID + bundle id ⇒ TCC grants and Keychain access survive updates.
...
### 9.1 Pinned official vendor helpers (`third_party/`)

The two subscription runtimes (A22) drive vendor binaries we do not build: the official **Codex app-server** and the official **Claude Code CLI**. One rule covers both.

- **Manifest, not binary.** `third_party/<helper>/manifest.toml` is committed and records, per helper and architecture: upstream version, source commit, release/artifact URLs, archive **SHA-256**, exact **byte count**, and licence URL + digest. No platform binary is ever committed.
- **`neo dev fetch-codex` / `neo dev fetch-claude-code`** download the pinned official artifact for the host architecture, verify byte count and SHA-256 **before** extracting, extract only the expected executable, check its `--version` against the manifest, and fetch the checksum-verified licence text beside it. Any mismatch aborts and leaves nothing on disk.
- **The vendor directory is gitignored.** A clean clone builds, tests and runs the TUI without it; only the two subscription runtimes are unavailable, and `neo doctor` says which helper is missing and which `fetch-*` command supplies it.
- **CI re-verifies** every manifest on each push (the dependency-manifest check) and again on `v*` tags before **signing each nested helper** with the Developer ID and hardened runtime (§9). Updating a helper is an explicit dependency PR that reruns the provider conformance suite (08).
- **Licences differ and the pipeline respects the difference.** Codex is Apache-2.0, so its binary and notices ship inside the bundle. The Claude Code CLI is published under Anthropic's own terms (`SEE LICENSE IN README.md`, not an SPDX open-source licence), so until redistribution is confirmed in writing the release build fetches it into the user's own helper home instead of embedding it, and Doctor reports it as absent rather than blocking the app *(verify against Anthropic's published terms before bundling)*.
- **Each helper owns its own credentials** in its own dedicated home and keychain entries (§6, K6); Starkbot reads neither, and only redacted account state reaches SQLite.

## 10. Dev workflow

| Mode | Use for | TCC / Keychain |
|---|---|---|
| **`neo` CLI** (`cargo run -p neo-cli -- …`) | headless subsystem work: doctor, keys, models, voice, nav, judge, run, scenario | grants attributed to the terminal — run from Terminal.app/iTerm, not an IDE terminal; keys from env |
| **`neo tui`** (`cargo run -p neo-cli -- tui`) | **the default surface for development and for every milestone's acceptance (P12)**: Conversation, Queue, Mind/trace and confirm cards over the real `Runtime`. A milestone is done when it works here; the desktop UI follows | same terminal grants and env keys as the CLI; panels, TCC prompts and the bridge are not exercised here |
...
**`neo doctor`** (also the Settings → Doctor tab; exit code ≠ 0 on any red): macOS version and arch · running from `/Applications`? · signature: identity, team id, hardened runtime, entitlements match the expected two · each pinned vendor helper (Codex app-server, Claude Code CLI) present, expected version/hash/licence, signature valid, and its protocol handshake reporting a compatible version — or absent with the `neo dev fetch-*` line (§9.1) · subscription accounts (ChatGPT, Claude) `connected / signed out / rate limited` plus redacted plan/workspace and reset (never tokens) · Accessibility `granted / stale` (+ the `tccutil` line) · `CGPreflightPostEventAccess` · Microphone status · input device present and default sample rate · Keychain read/write round-trip · each direct key's `KeyStatus` · OpenAI reachable, `/models` latency, API `sol-latest` resolution, configured ids present and not hidden · Anthropic reachable, `/v1/models` latency, configured id present · TypeSafe reachable + `ping` latency · `prices.json` age and signature · DB opens, `user_version`, `integrity_check`, WAL size, free disk · `worker.lock` holder · Chrome found, version, managed profile launches with a debugging pipe ·…
...
| **Key exfiltration** | Direct keys and the two OAuth credential blobs: Keychain only; `Secret` redaction + clippy-fenced `expose()`; the webview sees status only and has no HTTP capability to vendor hosts (CSP + Tauri capabilities allow IPC only); nothing is typed, spoken, shown or sent in a task; secure fields never read; no key, access token, refresh token or `id_token` in argv, logs, diagnostics, traces, snapshot tests or crash output — the OAuth blob is written to the Keychain by the exchange/refresh path and read only by the provider impl minting one `Authorization` header (K7). Vendor-CLI fallbacks: only the official helper handles those tokens, in its own dedicated Keychain/`CODEX_HOME`/`CLAUDE_CONFIG_DIR`; Neo accepts redacted account events and never reads their credential files or protocol fields carrying tokens. |
...
| `neo-media` | *(removed 2026-09-19)* fake `MediaBackend`; estimate → confirm threshold; studio index rebuild; take ids round-trip with `dmm` |
...
| `neo-tui` | deterministic **buffer snapshots** (`insta` over ratatui's `TestBackend`): a fixed 120 × 40 terminal, a scripted event stream (canned `AppEvent`s interleaved with key, paste and resize events), one snapshot per frame — every pane, task card, trace item and confirm state, plus the narrow layout. No wall clock, no terminal-capability probing, no real TTY, so the suite runs identically in CI. Command dispatch is asserted as typed commands sent to `Runtime`, never as state the TUI invented; §1 rule 3's grep proves no vendor host or secret literal appears in the crate |
...
On every push and PR: `cargo fmt --check` → `cargo clippy --workspace --all-targets -- -D warnings` → `cargo test --workspace` (offline; live tests are `#[ignore]`; the `neo-tui` buffer snapshots run here) → `cargo deny check` + `cargo audit` → rule greps (§1 rules 1, 3, 4 — rule 3's vendor-host/secret grep covers `neo-tui` and `ui/`) → vendor-helper manifest verification (§9.1) → `neo dev bindings && git diff --exit-code` → `pnpm i --frozen-lockfile && pnpm typecheck && pnpm test` → `cargo tauri build --debug --no-bundle` unsigned. Caches: cargo registry + a per-branch target dir. **Signing, notarization, DMG and updater artifacts run only on `v*` tags**, in a separate workflow with the Apple and updater secrets scoped to a protected environment. If Actions minutes are refused for the account, the same steps run from `neo dev ci` locally and the tag workflow is dispatched by hand.
...
| 4 | `OpenAiInference::list_models` + validator, `AnthropicInference::list_models` + validator (K6); pinned Codex app-server and Claude Code CLI manifests/checksums/licences (§9.1) + supervisor initialization/account/login/logout/rate-limit clients (no agent turns yet); `jev-nav::wire` promoted from the M0 spike with `ping`; registry for every runtime catalog (classify, resolve, hide-list, `models` cache); price fetch; `neo models` / `neo account` | both API keys validate; each subscription conformance fixture logs in without exposing a token; every runtime catalog resolves `sol-latest`; hidden ids never listed |
| 5 | `neo-ax::perm`, `neo-voice::perm`; `neo doctor` with the §10 list (media / Chrome checks report "not yet") | doctor is correct in all four TCC states and in every subscription-helper state (helper missing, signed out, connected, limited) |
| 6 | Tauri scaffold: `tauri.conf.json`, `Info.plist` strings, both entitlements files, plugins (single-instance first), Developer ID wired; bundle and sign `neo` + every redistributable nested helper (§9.1); signed debug build into `/Applications` | **Accessibility grant and direct-key Keychain access survive three rebuilds**; the two OAuth credential items survive too, each fallback helper's dedicated account survives, and no token appears anywhere but the Keychain |
| 7 | `Runtime` facade stub in `neo-agent`; `AgentRuntime` interface with stubs for all six runtimes (real turn loops land M5); `tauri-specta` bridge: `get_bootstrap`, settings, direct keys, subscription OAuth login + account events, models, permissions, `AppEvent` emitter; bindings drift check in CI | both front ends receive typed events from Rust; account snapshots are redacted and carry no token |
| 8 | `neo-tui` shell: Conversation / Queue / Mind panes, confirm cards and the connection + key screens over the same bootstrap and `AppEvent` stream, with buffer snapshots committed; then the webview shell: four Vite entries (three as stubs), store + `useAppEvents`, onboarding 1–6 against real checks, inference-connection choice / `KeyField` / typed-only branch / `PermissionRow` | **M1 is accepted in `neo tui` first (P12)**; afterwards a fresh user account completes onboarding in the app through any of the six inference paths |
| 9 | Settings: Connections & Keys, Models, General, Doctor; revoked-permission/account re-gating; updater keypair generated and backed up twice; tag workflow: vendor-helper manifest check → sign nested code → notarize → staple → DMG → `latest.json` | a tagged build installs on a second Mac, completes a subscription OAuth login into its own Keychain items (or connects the chosen fallback helper in that helper's own home), and passes `spctl` |
